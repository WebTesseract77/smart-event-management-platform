...
function formatSubmittedDate(raw?: string) {
  if (!raw) return "—";
  try {
...
export default function AdminUsersPage() {
  ...
  const [reviewStep, setReviewStep] = useState<"details" | "confirm-approve" | "reject">("details");
const [rejectRemark, setRejectRemark] = useState("");
const [reviewSubmitting, setReviewSubmitting] = useState(false);

  // --- Derived display values for the selected organizer request ---
 ...
 async function handleApproveRequest() {
  ...
  try {
    ...
    toast.success(`${applicantName} is now an Organizer`);
    await refreshUsers();
                ...
                {view === "users" ? (
                  ...
                  <div className="py-12 border border-dashed rounded-[22px] border-[#E8E1D5] w-full">
                    <EmptyState
                      ...
                      title={organizerRequests.length === 0 ? "No organizer requests" : "No matches"}
                      description={organizerRequests.length === 0 ? "Organizer applications will appear here once submitted." : "Try adjusting your search."}
                    />
                  </div>
                ) : (
                  <>
                      ...
                      {filteredRequests.map((request) => {
                        ...
                        return (
                          <div key={request.id} className="p-3 sm:p-4 rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4]/50 space-y-4 w-full min-w-0">
                            ...
                            <div className="pt-3 border-t border-[#E8E1D5]/60 flex justify-end w-full min-w-0">
                              <Button size="sm" className="w-full h-9 rounded-full bg-[#0F4D3F] text-xs text-white hover:bg-[#0B3E33] flex items-center justify-center px-4" onClick={() => openReviewDialog(request)}>
                          ...
                          {filteredRequests.map((request) => {
                            ...
                            return (
                              <TableRow key={request.id} className="transition-colors hover:bg-[#FBFAF7]">
                                ...
                                <TableCell className="pr-6 py-5 text-right">
                                  <Button size="sm" className="h-10 rounded-full bg-[#0F4D3F] px-4 text-xs text-white hover:bg-[#0B3E33]" onClick={() => openReviewDialog(request)}>
...
>
  {dialogConfirmLabel}
      ...
      >
        <DialogContent className="w-[95vw] max-w-3xl max-h-[90vh] overflow-hidden p-0 rounded-[1.5rem] sm:rounded-[2rem] border border-white/70 bg-white shadow-2xl">
          <div className="flex h-full max-h-[90vh] min-h-0 flex-col overflow-hidden">
            {selectedRequest && reviewStep === "details" && (
              <>
                <DialogHeader className="shrink-0 border-b border-[#E8E1D5] px-5 sm:px-6 pt-5 sm:pt-6 pb-4">
                  <div className="flex items-start justify-between gap-3 min-w-0">
                    <div className="flex items-center gap-3 min-w-0">
                      <Avatar className="h-12 w-12 shrink-0 border border-[#E8E1D5] bg-[#0F4D3F] text-white shadow-sm">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 truncate text-sm text-[#5E665F]">{applicantEmail}</p>
                      </div>
                    </div>
                    {(() => {
                ...
                <div className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden px-5 sm:px-6 py-5 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 min-w-0">
                    <div className="min-w-0 overflow-hidden rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4] p-4">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#8B938C]">
                        Organization
                      </p>
                      <p className="mt-1 text-sm font-medium text-[#183028] break-words">
                    ...
                    <div className="min-w-0 overflow-hidden rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4] p-4">
                      ...
                      <p className="mt-1 text-sm font-medium text-[#183028]">
                  ...
                  <div className="min-w-0 overflow-hidden rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4] p-4">
                    ...
                    {(selectedRequest as any).portfolio_url ? (
                      <a
                        href={(selectedRequest as any).portfolio_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-1 block text-sm font-medium text-[#0F4D3F] underline break-all hover:text-[#0B3E33]"
                      >
                        {(selectedRequest as any).portfolio_url}
                      </a>
                    ) : (
                      <p className="mt-1 text-sm text-[#5E665F]">—</p>
                    )}
                  </div>
                  ...
                  <div className="min-w-0 overflow-hidden rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4] p-4">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5 min-w-0">
                      {applicantCategories.length > 0 ? (
                        applicantCategories.map((category: string) => (
                          <Badge
                            key={category}
                            className="rounded-full border border-[#E8E1D5] bg-white px-3 py-1 text-xs font-medium text-[#183028] break-words"
                          >
                            {category}
                          </Badge>
                        ))
                  ...
                  <div className="min-w-0 overflow-hidden rounded-2xl border border-[#E8E1D5] bg-[#FAF8F4] p-4">
                    ...
                    <p className="mt-1 text-sm leading-6 text-[#183028] break-words whitespace-pre-wrap">
                      {(selectedRequest as any).reason || "—"}
                    </p>
                  </div>

                  {selectedRequest.status === "rejected" && (selectedRequest as any).admin_remark && (
                    <div className="min-w-0 overflow-hidden rounded-2xl border border-[#FCDEDE] bg-[#FDECEC] p-4">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#B42318]">
                        Admin Remark
                      </p>
                      <p className="mt-1 text-sm leading-6 text-[#7A241A] break-words whitespace-pre-wrap">
                ...
                <DialogFooter className="shrink-0 border-t border-[#E8E1D5] px-5 sm:px-6 py-4 flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
            ...
            {selectedRequest && reviewStep === "confirm-approve" && (
              <>
                <DialogHeader className="shrink-0 border-b border-[#E8E1D5] px-5 sm:px-6 pt-5 sm:pt-6 pb-4">
                  <DialogTitle className="text-xl sm:text-2xl font-bold tracking-tight text-[#183028]">
                    Approve {applicantName}'s request?
                  </DialogTitle>
                </DialogHeader>
                <div className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden px-5 sm:px-6 py-5">
